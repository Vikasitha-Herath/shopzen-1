...
// ─── applyTheme ─────────────────────────────────────────────────────────────
// Exported so admin Settings page can call it directly on preview / save.
...
// ─── Module-level immediate bootstrap ───────────────────────────────────────
// Runs the moment this JS module is evaluated — before ANY React component
// renders. Applies cached theme (or default) so CSS vars are correct before
// the first paint, overriding any :root defaults in index.css.
...
export const ThemeProvider = ({ children }) => {
  ...
  // Re-applies cached theme immediately on mount to cover any CSS cascade race.
  useLayoutEffect(() => {
    const cached = readCache();
    applyTheme(cached); // safe to call with null — applies defaults
  }, []);

  const loadAndApply = useCallback(async () => {
    try {
      const { data } = await API.get('/settings');
      // Guard against HTML error responses (happens when API URL is wrong in production)
