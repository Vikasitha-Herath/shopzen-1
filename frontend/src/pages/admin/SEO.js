...
const Pill = ({ ok, label }) => (
  <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-semibold ${ok ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
    {ok ? '✓' : '✗'} {label}
  </span>
);
...
export default function AdminSEO() {
  ...
  const runVitalsCheck = async () => {
    ...
    const hasPixel     = !!settings.facebookPixel;
      ...
      {tab === 'files' && (
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            {sitemapXml ? (
              <>
                <div className="bg-gray-50 rounded-xl p-3 border border-gray-200 mb-3 flex items-center justify-between flex-wrap gap-2">
                  <div>
                    <p className="text-xs font-bold text-gray-500">Sitemap URL</p>
                    <p className="font-mono text-sm text-gray-700">{settings.siteUrl || 'https://yourstore.com'}/sitemap.xml</p>
                  </div>
                  <CopyBtn text={`${settings.siteUrl || 'https://yourstore.com'}/sitemap.xml`}/>
                </div>
                <textarea value={sitemapXml} readOnly rows={15} className="form-input resize-none font-mono text-xs bg-gray-50"/>
                <p className="text-xs text-amber-600 mt-2 bg-amber-50 border border-amber-100 rounded-lg p-2.5">
                  ⚠️ <strong>Next step:</strong> Download sitemap.xml and place it in your server's public root. Then submit the URL to Google Search Console → Sitemaps.
                </p>
              </>
            ) : (
              <div className="text-center py-8 text-gray-400">
                <div className="text-4xl mb-2">🗺️</div>
                <p className="text-sm">Click "Generate Sitemap" to create one from your products and categories</p>
                <p className="text-xs mt-1">{products.length} products · {categories.length} categories found</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <p className="text-xs text-blue-600 mt-2 bg-blue-50 border border-blue-100 rounded-lg p-2.5">
              💡 Place robots.txt in your server's root directory at <strong>{(settings.siteUrl || 'https://yourstore.com') + '/robots.txt'}</strong>
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              {[
                { done: !!sitemapXml, step: '1', title: 'Generate sitemap.xml', detail: 'Click the button above to create sitemap' },
                ...
                { done: false, step: '4', title: 'Place sitemap.xml on your server', detail: 'Download and upload to root directory of your domain' },
                { done: false, step: '5', title: 'Submit sitemap in Google Search Console', detail: 'GSC → Sitemaps → Add your sitemap URL' },
                { done: false, step: '6', title: 'Submit robots.txt on your server', detail: 'Download and upload to root directory' },
              ].map(item => (
