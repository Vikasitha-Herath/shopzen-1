...
import {
  ...
  GlowOrb, ScrollProgressBar, FloatingShapes
...
const HeroSlider = ({ banners, settings, campaign, anim }) => {
  ...
  const orbs       = [useRef(null), useRef(null), useRef(null), useRef(null)];
    ...
    gsap.timeline({ onComplete: () => {
      ...
      gsap.fromTo([...textRef.current.children], enterMap[style]||enterMap['3d'],
        { opacity:1, y:0, x:0, rotateX:0, rotateY:0, scale:1, duration:0.85, stagger:0.12, ease:'power3.out',
          onComplete: () => { setTrans(false); startTimer(); } }
      );
  ...
  const startTimer = useCallback(() => {
    ...
    timerRef.current = setInterval(() => setCurrent(c => { const n=(c+1)%slides.length; goTo(n); return c; }), anim.heroInterval||6000);
  }, [anim.heroAutoplay, anim.heroInterval, slides.length, goTo]);
...
const PromoBanner = ({ banner, tall=false }) => {
  ...
  useEffect(() => {
    ...
    wrapRef.current.addEventListener('mouseenter',enter);
    return()=>wrapRef.current?.removeEventListener('mouseenter',enter);
  },[config.bannerShine]);
      ...
      <section key="featured" className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {[...Array(8)].map((_,i)=><div key={i} className="skeleton" style={{ aspectRatio:'0.88', borderRadius:20 }}/>)}
          </div>
