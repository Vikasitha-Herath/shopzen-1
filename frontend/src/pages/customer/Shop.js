...
export default function Shop() {
  ...
  useEffect(() => { fetchProducts(); }, [page, category, search, sortBy, onSale, inStock, priceMin, priceMax]);
          ...
          <div className="flex-1 min-w-0">
            {loading ? (
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {[...Array(12)].map((_,i)=><div key={i} className="skeleton" style={{aspectRatio:'0.85',borderRadius:20}}/>)}
              </div>
            ) : products.length === 0 ? (
