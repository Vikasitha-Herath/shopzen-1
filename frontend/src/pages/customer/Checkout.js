...
export default function Checkout() {
  ...
  // Calculate delivery fee from selected service
  const selectedDeliveryService = deliveryServices.find(s => s.code === selectedDelivery);
  const deliveryRate = selectedDeliveryService?.rates?.[0];
              ...
              <div className="rounded-2xl border border-gray-100 p-4" style={{ background: 'var(--card-bg)' }}>
                ...
                <div className="space-y-2">
                  {deliveryServices.map(svc => {
                    ...
                    const cost = rate ? (rate.freeAbove && subtotal >= rate.freeAbove ? 0 : rate.price) : 0;
                    return (
                      <label key={svc.code} className={`flex items-center justify-between gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedDelivery === svc.code ? 'bg-primary/5' : 'border-gray-200 hover:border-gray-300'}`}
                        style={selectedDelivery === svc.code ? { borderColor: 'var(--color-primary)' } : {}}>
                        <div className="flex items-center gap-2">
                          <input type="radio" name="delivery" value={svc.code} checked={selectedDelivery === svc.code} onChange={() => setSelectedDelivery(svc.code)} style={{ accentColor: 'var(--color-primary)' }} />
                          <div>
                            <p className="text-sm font-semibold text-gray-800">{svc.name}</p>
                            <p className="text-xs text-gray-500">{svc.estimatedDays}</p>
                          </div>
                        </div>
                        <span className={`text-sm font-bold ${cost === 0 ? 'text-green-600' : 'text-gray-800'}`}>
            ...
            <div className="rounded-2xl border border-gray-100 p-5" style={{ background: 'var(--card-bg)' }}>
              ...
              <div className="space-y-3">
                ...
                {settings?.bankTransferEnabled !== false && (
                  <label className={`flex items-start gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod==='bank_transfer'?'bg-primary/5':'border-gray-200 hover:border-gray-300'}`}
                    style={paymentMethod==='bank_transfer'?{borderColor:'var(--color-primary)'}:{}}>
                    <input type="radio" name="payment" value="bank_transfer" checked={paymentMethod==='bank_transfer'} onChange={()=>setPaymentMethod('bank_transfer')} className="mt-1 flex-shrink-0" style={{accentColor:'var(--color-primary)'}}/>
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-lg">🏦</span>
                        <p className="font-semibold text-sm text-gray-800">Direct Bank Transfer</p>
                      </div>
                      {paymentMethod==='bank_transfer' && (
                        <div className="mt-2.5 bg-gray-50 rounded-xl p-3 space-y-1.5 border border-gray-100">
                          <p className="text-xs text-gray-500 mb-2">Transfer using your order number as reference:</p>
                          {settings?.bankName && <p className="text-xs"><span className="text-gray-400 w-24 inline-block">Bank:</span><span className="font-semibold text-gray-700">{settings.bankName}</span></p>}
                          {settings?.bankAccountName && <p className="text-xs"><span className="text-gray-400 w-24 inline-block">Account:</span><span className="font-semibold text-gray-700">{settings.bankAccountName}</span></p>}
                          {settings?.bankAccountNumber && <p className="text-xs"><span className="text-gray-400 w-24 inline-block">Number:</span><span className="font-mono font-bold text-gray-900 bg-white px-1.5 rounded border border-gray-200">{settings.bankAccountNumber}</span></p>}
                          {settings?.bankBranch && <p className="text-xs"><span className="text-gray-400 w-24 inline-block">Branch:</span><span className="font-semibold text-gray-700">{settings.bankBranch}</span></p>}
                        </div>
                      )}
                    </div>
                  </label>
                ...
                {settings?.codEnabled !== false && (
                  <label className={`flex items-start gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod==='cod'?'bg-primary/5':'border-gray-200 hover:border-gray-300'}`}
                    style={paymentMethod==='cod'?{borderColor:'var(--color-primary)'}:{}}>
                    <input type="radio" name="payment" value="cod" checked={paymentMethod==='cod'} onChange={()=>setPaymentMethod('cod')} className="mt-1 flex-shrink-0" style={{accentColor:'var(--color-primary)'}}/>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-lg">💵</span>
                        <p className="font-semibold text-sm text-gray-800">Cash on Delivery</p>
                      </div>
                      {paymentMethod==='cod' && <p className="text-xs text-gray-500 mt-1 ml-6">Pay cash when your order arrives.</p>}
                    </div>
                  </label>
                )}
                ...
                {gateways.map(gw => (
                  <label key={gw.gateway} className={`flex items-start gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod===gw.gateway?'bg-primary/5':'border-gray-200 hover:border-gray-300'}`}
                    style={paymentMethod===gw.gateway?{borderColor:'var(--color-primary)'}:{}}>
                    <input type="radio" name="payment" value={gw.gateway} checked={paymentMethod===gw.gateway} onChange={()=>setPaymentMethod(gw.gateway)} className="mt-1 flex-shrink-0" style={{accentColor:'var(--color-primary)'}}/>
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        {gw.logo ? (
                          <img src={gw.logo} alt={gw.displayName} className="h-5 object-contain" />
                        ) : (
                          <span className="text-lg">🔌</span>
                        )}
                        <p className="font-semibold text-sm text-gray-800">{gw.displayName}</p>
                        {!gw.isLive && <span className="text-xs bg-amber-100 text-amber-700 px-1.5 rounded font-medium">Sandbox</span>}
                      </div>
                      {paymentMethod===gw.gateway && (
                        <p className="text-xs text-gray-500 mt-1 ml-6">
                          {gw.gateway==='payhere' && 'You will be redirected to PayHere secure checkout.'}
