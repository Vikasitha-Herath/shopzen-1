...
const Header = ({ settings, campaign }) => {
  ...
  const announcement = campaign?.announcement || (settings?.freeDeliveryThreshold ? `🚚 Free delivery on orders over ${settings?.currencySymbol||'Rs.'} ${(settings.freeDeliveryThreshold||5000).toLocaleString()} — Shop now!` : null);

  return (
    <header ref={headerRef} className={`sticky top-0 z-[45] transition-all duration-500 ${scrolled ? 'shadow-2xl' : ''}`}
      style={{background: scrolled ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.98)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', borderBottom: scrolled ? 'none' : '1px solid rgba(0,0,0,0.05)'}}>
      
      {/* Ticker announcement */}
      {announcement && (
        <div className="announcement-bar overflow-hidden py-1.5 text-xs font-bold text-white"
          style={{background: campaign?.announcementBg || 'var(--theme-gradient)'}}>
          <div className="ticker-wrap">
            <div className="ticker-track">
              {[...Array(6)].map((_, i) => <span key={i} className="px-8">{announcement} &nbsp;✦&nbsp;</span>)}
            </div>
          </div>
        </div>
      )}
